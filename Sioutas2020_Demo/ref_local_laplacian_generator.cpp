...
public:
    ...
    void generate() {
        ...
        Expr level = k * (1.0f / (8 - 1));
        Expr idx = gray(x, y)*cast<float>(8-1)*256.0f;
        idx = clamp(cast<int>(idx), 0, (8-1)*256);
        ...
        for (int j = 0; j < J; j++) {
            ...
            Expr level = inGPyramid[j](x, y) * cast<float>(8-1);
            Expr li = clamp(cast<int>(level), 0, 8-2);
...
//
        ...
        if (auto_schedule) {
         outLPyramid[7].bound_extent(x,16);//.bound(y,0,32);
//	}   // Nothing.
